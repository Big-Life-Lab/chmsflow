...
test_that("derive_alcohol_risk returns correct scores", {
  ...
  expect_equal(df_alc %>% dplyr::mutate(low_drink_score = derive_alcohol_risk(clc_sex, alc_11, alcdwky)) %>% dplyr::pull(low_drink_score), expected_output_alc)
})
...
test_that("derive_alcohol_risk_detailed returns correct scores", {
  ...
  expect_equal(df_alc1 %>% dplyr::mutate(low_drink_score1 = derive_alcohol_risk_detailed(clc_sex, alc_11, alcdwky, alc_17, alc_18)) %>% dplyr::pull(low_drink_score1), expected_output_alc1)
})
