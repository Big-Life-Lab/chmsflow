...
test_that("derive_cvd_personal_history returns correct CVD personal history", {
  ...
  expect_equal(derive_cvd_personal_history(1, 1, 1), 1)
  expect_equal(derive_cvd_personal_history(1, 1, 2), 1)
  expect_true(haven::is_tagged_na(derive_cvd_personal_history(6, 2, 2), "a"))
  expect_true(haven::is_tagged_na(derive_cvd_personal_history(2, 6, 2), "a"))
  expect_true(haven::is_tagged_na(derive_cvd_personal_history(2, 2, 6), "a"))
  ...
  expect_equal(df |> dplyr::mutate(cvd = derive_cvd_personal_history(CCC_81, CCC_91, CCC_92)) |> dplyr::pull(cvd), c(1, 1, 1))
})
...
test_that("derive_cvd_family_history returns correct CVD family history", {
  ...
  expect_equal(derive_cvd_family_history(1, 59, 2, NA), 1)
  expect_equal(derive_cvd_family_history(1, 60, 2, NA), 2)
  expect_equal(derive_cvd_family_history(2, NA, 1, 59), 1)
  expect_equal(derive_cvd_family_history(2, NA, 1, 60), 2)
  expect_equal(derive_cvd_family_history(1, 50, 1, 55), 1)
  expect_equal(derive_cvd_family_history(1, 65, 1, 70), 2)
  expect_equal(derive_cvd_family_history(1, 0, 2, NA), 1)
  expect_true(haven::is_tagged_na(derive_cvd_family_history(1, 80, 2, NA), "b"))
  expect_true(haven::is_tagged_na(derive_cvd_family_history(1, -1, 2, NA), "b"))
