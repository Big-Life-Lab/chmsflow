...
#'
#' @description This function creates a derived variable (pack_years_der) that measures an individual's smoking pack-years based on various CHMS smoking
#' variables. This is a popular variable used by researchers to quantify lifetime exposure to cigarette use.
#'
...
#'       - Daily smoker: (CLC_AGE - SMK_52) * (SMK_31 / 20) (minimum value 0.0137)
#'       - Occasional smoker (former daily): ((CLC_AGE - SMK_52 - SMKDSTP) * (SMK_23 / 20)) + (pmax((SMK_41 * SMK_11 / 30), 1) * SMKDSTP)
#'       - Occasional smoker (never daily): (pmax((SMK_41 * SMK_11 / 30), 1) / 20) * (CLC_AGE - SMK_21)
#'       - Former daily smoker (non-smoker now): pmax(((CLC_AGE - SMK_52 - SMKDSTP) * (SMK_23 / 20)), 0.0137)
...
#' @examples
#' 
#' # Example 1: Calculate pack years for a daily smoker who started at age 20 and smokes 30 cigarettes per day.
#' pack_years_fun(SMKDSTY = 1, CLC_AGE = 40, SMKDSTP = NA, SMK_52 = 20, SMK_31 = 30, SMK_41 = NA, SMK_53 = NA, SMK_23 = NA, SMK_21 = NA, SMK_11 = NA)
#' # Output: 4 (pack years)
#'
...
pack_years_fun <- function(SMKDSTY, CLC_AGE, SMKDSTP, SMK_52, SMK_31, SMK_41, SMK_53, SMK_23, SMK_21, SMK_11) {
  ...
  if (SMKDSTY == 1) {
    pack_years <- pmax(((CLC_AGE - SMK_52) * (SMK_41 / 20)), 0.0137)
  }
  # PackYears for Occasional Smoker (former daily)
  else if (SMKDSTY == 2) {
    pack_years <- pmax(((CLC_AGE - SMK_52 - SMKDSTP) * (SMK_23 / 20)), 0.0137) +
      (pmax((SMK_31 * SMK_11 / 30), 1) * SMKDSTP)
  }
  # PackYears for Occasional Smoker (never daily)
  else if (SMKDSTY == 3) {
    pack_years <- (pmax((SMK_31 * SMK_11 / 30), 1) / 20) * (CLC_AGE - SMK_21)
  }
