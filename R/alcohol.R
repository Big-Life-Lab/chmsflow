...
#' @examples
#'
#' # Example: A male respondent who drank 3 standard drinks per week and drank within past year.
...
low_drink_score_fun1 <- function(CLC_SEX, ALCDWKY, ALC_17, ALC_11) {
  ...
  if (any(c(CLC_SEX, ALCDWKY) < 0, na.rm = TRUE)) {
    return(haven::tagged_na("b"))
  }

  # Check if any inputs are NA
  if (is.na(CLC_SEX) || is.na(ALCDWKY)) {
    return(haven::tagged_na("b"))
  }
