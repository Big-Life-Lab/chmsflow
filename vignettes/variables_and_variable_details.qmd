...
This vignette is a column-by-column reference for both files. For an explanation of how these files fit into the harmonization workflow, see `vignette("methodology")`.
...
| Column | Description |
|--------|-------------|
| `variable` | Name of the harmonized variable |
| `label` | Short label |
| `labelLong` | Detailed label |
| `section` | Broad grouping (e.g., sociodemographics, health behaviour, health status) |
| `subject` | Specific topic (e.g., age, smoking, blood pressure) |
| `variableType` | `Categorical` or `Continuous` |
| `units` | Units of the harmonized variable, or `N/A` |
| `databaseStart` | CHMS cycles that contain the variable |
| `variableStart` | Source variable names, with cycle-specific exceptions where needed |
...
See `vignette("derived_variables")` for details on how derived variables work.
