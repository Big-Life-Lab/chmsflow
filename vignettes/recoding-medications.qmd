...
2. Create synthetic data for cycle 1 and cycle 3.
```{r}
# wide cycle1 data
cycle1 <- data.frame(
  clinicid = c(1, 2, 3, 4),
...
5. Determine hypertension status by recoding medications 
```{r}
# Cycles 1-2
...
# At RDC, load in medication data with same name as cycle data (ie. cyclex) to recode medication variables
...
# At RDC, after recoding medication variables and before merging, load in original data with typical cyclex name
