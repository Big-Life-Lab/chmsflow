...
2. Create synthetic data for cycle 1 and cycle 3, with each cycle's medication data being separate from their cycle.
```{r}
# Cycle 1 - wide medication data
cycle1_survey <- data.frame(
  clinicid = c(1, 2, 3, 4),
...
```{r}
# Always load in medication data with same name as cycle data (ie. "cyclex") in order to recode medication variables
cycle1 <- cycle1_meds
cycle3 <- cycle3_meds
...
4. Merge recoded medication data
```{r}
...
cycle1 <- merge(cycle1_survey, cycle1_htn_medication_data, by = "clinicid")
...
# Call overall data "cyclex" in order to recode other variables afterwards
cycle3 <- merge(cycle3_survey, cycle3_htn_medication_data, by = "clinicid")
