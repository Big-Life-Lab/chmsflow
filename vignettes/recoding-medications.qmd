...
2. Load worksheets (variables and variable details sheets) and dplyr.
```{r}
...
3. Create synthetic data for cycle 2 and cycle 3, with each cycle's medication data being separate. 

Medication data object always has to be called "cyclex_meds" in order for recoding to work, with rest of the data being called "cyclex" as stated in recoding-variables.qmd. 
...
4. Recode medication variables for individual cycles.
```{r}
# Recoding basic variables from cycles 1 and 3
cycle2_medication_data <- recodeflow::rec_with_table(cycle2_meds, c("clinicid", "atc_101a", "mhr_101b"), variable_details = variable_details)
head(cycle2_medication_data)

cycle3_medication_data <- recodeflow::rec_with_table(cycle3_meds, c("clinicid", "meucatc", "npi_25b"), variable_details = variable_details)
...
cycle2_ace_medication_data <- recodeflow::rec_with_table(cycle2_meds, c("clinicid", "acemed", recodeflow:::select_vars_by_role("Drugs", variables)), variable_details = variable_details)
...
cycle3_ace_medication_data <- recodeflow::rec_with_table(cycle3_meds, c("clinicid", "meucatc", "npi_25b", "acemed"), variable_details = variable_details)
head(cycle3_ace_medication_data)
```

5. Merge recoded medication data from different cycles.
```{r}
...
6. If you need medication variables in your analyis (especially to derive other variables), always recode them before recoding other variables.
...
cycle2_htn_medication_data <- recodeflow::rec_with_table(cycle2_meds, c("clinicid", recodeflow:::select_vars_by_role("Drugs", variables)), variable_details = variable_details)
...
cycle2_htn_data <- recodeflow::rec_with_table(cycle2, c(recodeflow:::select_vars_by_role("Test", variables)), variable_details = variable_details)
head(cycle2_htn_data)

# Cycles 3-6
# Recode medication variables first - - anymed for hypertension and diab_drug for diabetes which is involved in determining hypertension status
cycle3_htn_medication_data <- recodeflow::rec_with_table(cycle3_meds, c("clinicid", "meucatc", "npi_25b", "anymed", "diab_drug"), variable_details = variable_details)
...
# Recode hypertension (and diabetes) status using recoded, dummy medication variables
cycle3_htn_data <- recodeflow::rec_with_table(cycle3, c(recodeflow:::select_vars_by_role("Test", variables)), variable_details = variable_details)
