...
For variables that depend on medication status (e.g., hypertension, diabetes), use `recode_after_meds()` instead of `rec_with_table()`. See `vignette("recoding_medications")` and `vignette("analysis_walkthrough")` for the full workflow.

## Creating a derived variable

To add a new derived variable to chmsflow, you need to create a harmonized set of input variables and an R function that computes the derived value. See `vignette("how_to_add_variables")` for step-by-step instructions.

For details on the metadata schema, see `vignette("variables_and_variable_details")`.
