...
This means: use `amsdmva1` for cycle 1, and `ammdmva1` for all other cycles. The `recodeflow` package reads this format and applies the correct mapping automatically. For the full list of variable naming patterns, see the [CHMS database configuration](../inst/metadata/schemas/chms/chms_database_config.yaml).
...
For detailed schema documentation, see `vignette("variables_and_variable_details")`.
...
This distinction matters for analysis. For example, a respondent who was never asked about smoking (valid skip) should be treated differently from one who refused to answer (missing). See `vignette("tagged_na_usage")` for a full explanation.
...
The `DerivedVar::` prefix in `variableStart` lists the input variables that must be present in the data before the function can run. See `vignette("derived_variables")` for details.
...
- **Survey weights and bootstrap variance estimation** are outside chmsflow's scope. Researchers must apply appropriate weights from the CHMS master file.
...
- Manuel, D. et al. CHMS user guide materials. Available at [OSF](https://osf.io/v27mw/files/v8ahd).
